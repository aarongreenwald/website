...
By integrating [Sentry](https://sentry.io) data with [New Relic](https://newrelic.com) dashboards, you can get both deep analysis of your React Native app’s errors AND easy to digest, high level dashboards that quickly tell you what’s going on. No sacrifices or tradeoffs. Want to jump to the technical bits? Skip to [here](#solution). Setup instructions for your own project are available in the [GitHub](https://github.com/wix/sentry-monitor) [README](https://github.com/wix/sentry-monitor/README.md).
...
That your app is full of bugs isn’t the problem. That’s normal. The problem is that you have no way of knowing just how buggy it is if you don’t monitor errors, crashes, business KPIs, and gather general BI. The fundamental principles of [devops](https://en.wikipedia.org/wiki/DevOps) is that your work isn’t finished when you ship, it’s just getting started.
...
* And how can I fix it? (Stack trace)

In other words, Sentry’s web app is focused on drilling in to a specific issue/error and studying the events that occur that match that issue. That’s a really important thing to have, but it’s only half of what I need.

When I walk into my office in the morning, I want to see a big number on the wall that represents our error count in the last hour. I’d like to see a graph that shows the error trend in the last few hours compared with last week at the same time (since usage is often cyclical) and maybe a longer term trend graph.
...
It took me only a few hours to whip up a script that uses the Sentry API to retrieve the last five minutes’ worth of data, filter and aggregate it, and report it to New Relic and Anodot using their APIs. Then another view hours to build my dashboards and alerts, and I’m done.

Then I spent a while working on that script to turn it into something more robust and generic so that you can use it as well. It’s still a work in progress, and I welcome your assistance in the form of issues and PRs on Github.

You can head over to the README for the full instructions on how to get started, but here’s the basic idea:
...
const config = {
  ...
  WSM_AUTH_KEY: 'optional_secret_key_for_security',
...
When you feel confident that the setup is correct for you, remove the `debug` query param and the data will be sent to New Relic and Anodot.

*Note:* you need the `WSM_AUTH_KEY` header if you're going to host the web server on a different server than the cron job and expose the web server publicly. The web server will reject any request that doesn't come with the secret key you set.
